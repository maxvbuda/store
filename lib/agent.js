...
function create(env, send, readBody, browser, askModel, onError) {
  ...
  async function loop(goal, maxSteps) {
    // maxSteps '∞' = unbounded: the run ends when the goal is met, you press
    // Stop, a wall needs a human, or something actually fails — not on a
    // counter. A numeric maxSteps (explicit API callers) still bounds it.
    const unlimited = maxSteps === '∞';
    const history = [];
    for (let i = 1; unlimited || i <= maxSteps; i++) {
      ...
      let out;
      try {
        // Only the recent action window rides in the prompt — an unbounded
        // run would otherwise grow its prompt every step until it starves
        // the model. The page state is always current; old actions age out.
          ...
          prompt: 'Goal: ' + goal + '\n\n' + describe(page)
            + (recent.length ? '\n\nActions so far'
               + (history.length > recent.length
                  ? ' (earliest ' + (history.length - recent.length) + ' omitted)' : '')
               + ':\n' + recent.join('\n') : '')
  ...
  async function handle(req, res, url) {
    ...
    if (action === 'start' && req.method === 'POST') {
      ...
      let body;
      try { body = await readBody(req); } catch (e) { send(res, 400, { error: String(e.message) }); return true; }
      // Bounded: an unbounded goal rides into every status poll, every log
      // entry and every model prompt — one giant paste degraded the whole UI.
      // 6000 chars is beyond any real instruction and safely under the
      // model-prompt slice.
      ...
      stopRequested = false;
      clearInterval(lrPoll); lrPoll = null;
      // No step limit by default — the run ends on done/stop/wall/failure.
      // An explicit numeric maxSteps from an API caller still bounds it.
      const cap = Number(body.maxSteps);
      Object.assign(run, {
        id: run.id + 1, goal, status: 'running', step: 0,
        maxSteps: cap > 0 ? Math.min(1000, cap) : '∞',
        answer: '', error: '', startedAt: new Date().toISOString(), endedAt: null,
        log: [], backend: 'builtin',
      });
      note('goal', goal);
      // Say which brain is driving, up front. A run on the fallback loop
      // looked identical to one on the user's own architecture — they watched
      // the built-in loop botch a longrun-shaped task believing it was theirs.
      if (!longrun.configured()) {
        note('brief', 'running on the BUILT-IN loop — the longrun backend is not configured on this machine');
      }
