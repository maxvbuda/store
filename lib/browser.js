...
function create(env, send, readBody) {
    ...
    starting = (async () => {
      ...
      child = spawn('python3', args, {
        ...
        env: Object.assign({}, process.env, browsersEnv, {
          // Every other store (accounts, error log, agent brief) relocates
          // with DATA_DIR — the profile must too, or DEPLOY.md's "mount a
          // disk at /data and set DATA_DIR=/data to keep the agent's logins"
          // is false and sessions silently vanish on redeploy. An explicit
          // BROWSER_PROFILE still wins over the derived path.
          BROWSER_PROFILE: env('BROWSER_PROFILE', '')
            || path.join(env('DATA_DIR', path.join(__dirname, '..', '.data')), 'browser-profile'),
          BROWSER_START_URL: env('BROWSER_START_URL', '')
            || ('http://127.0.0.1:' + (process.env.PORT || 8787) + '/agent-start.html'),
          // process.env alone only has real OS env vars — a KERNEL_API_KEY
          // set solely in .env (the local-dev case) wouldn't otherwise reach
          // this child process. Runs Chromium on Kernel's infrastructure
          // instead of launching it locally when set.
          KERNEL_API_KEY: env('KERNEL_API_KEY', ''),
        }),
  ...
  async function handle(req, res, url) {
    ...
    // Live view: an endless multipart/x-mixed-replace stream of JPEG frames.
    // No timeout on this fetch — the response never ends by design. When the
    // viewer disconnects we abort the upstream fetch so the sidecar's stream
    // thread is released instead of writing frames into the void forever.
    if (action === 'stream') {
      const upstream = new AbortController();
      res.on('close', () => upstream.abort());
      let r;
      try {
        r = await fetch(BASE + '/stream', { signal: upstream.signal });
      } catch (e) {
        send(res, 502, { error: String(e.message || e) });
        return true;
      }
      ...
      try {
        ...
        // Client hung up (our abort fired) or the sidecar died mid-stream —
        // either way there is nobody left to send an error to.
        res.end();
      }
      return true;
    }

    const ALLOWED = ['goto', 'click', 'type', 'key', 'scroll', 'back', 'input', 'wheel'];
