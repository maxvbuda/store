...
function createStore(dataDir) {
  ...
  return {
    ...
    userForToken(token) {
      ...
      if (!s) return null;
      // Expire server-side, matching the Supabase store's TTL cutoff.
      if (Date.now() - s.createdAt > SESSION_TTL_DAYS * DAY * 1000) {
...
function create(env, send, readBody) {
  ...
  const publicUser = (u) => {
    ...
    return { id: u.id, email: u.email, createdAt: u.createdAt, setup: u.setup ? setupRest : null };
  };
