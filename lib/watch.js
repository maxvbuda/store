...
function create(env, send, readBody, browser) {
  ...
  async function check() {
    // Overlapping passes stack up fetches against a hung sidecar. One at a time.
  ...
  async function runCheck() {
    ...
    try {
      const state = await withTimeout(browser.state(), 12000, 'state');
      out.browser = state;
      ...
      if (notRunning) {
        ...
        try {
          await withTimeout(browser.read(), 60000, 'read');
          out.browser = await withTimeout(browser.state(), 12000, 'state');
      ...
      if (dead) {
        ...
        const fixed = await withTimeout(browser.restart(), 90000, 'restart');
        out.repaired = fixed;
        healed += fixed ? 1 : 0;
        if (fixed) {
