...
// Display clamp. Goals and answers are interpolated into headlines, digest
// copy and status rows — one giant paste used to stretch those elements and
// wreck the whole layout. Data stays intact; only what is SHOWN is bounded.
...
class Component extends DCLogic {
  ...
  sendChat() {
    ...
    fetch('/api/agent/start', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ goal: text }),   // no step limit — runs until done or stopped
  ...
  async submitGoal() {
    ...
    try {
      const res = await fetch('/api/agent/start', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ goal }),   // no step limit — runs until done or stopped
      });
  ...
  renderVals() {
    ...
    const desks = [
      {
        name: 'Agent',
        task: clamp(r.goal, 90) || 'No goal set',
        detail: running ? ('Step ' + r.step + ' of ' + r.maxSteps)
          : clamp(r.answer || r.error || 'Waiting for a goal', 110),
        queue: running
          ? (r.maxSteps === '∞' ? '∞' : String(Math.max(0, (r.maxSteps || 0) - (r.step || 0))))
          : '0',
    ...
    return {
      ...
      goalTitle: clamp(r.goal, 80) || 'No goal yet — press Run goal',
      ...
      digestSummary: (function () {
        ...
        if (r.status === 'running') {
          return 'Working on "' + clamp(r.goal, 140) + '" — step ' + r.step + ' of ' + r.maxSteps
        ...
        if (r.status === 'done') return 'Finished "' + clamp(r.goal, 120) + '". ' + clamp(r.answer, 400);
        if (r.status === 'failed') return 'Failed on "' + clamp(r.goal, 120) + '": ' + clamp(r.error, 300);
        return 'Run ' + r.status + '.';
      })(),
      ...
      stepText: clamp(run.answer || run.error || run.goal, 160) || 'No goal set',
